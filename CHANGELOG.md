...
- **Global Communities leaderboard:** server names are no longer all wrapped in bold (which broke Discord markdown). Only the arcade you’re viewing from is shown in **bold** when it appears in the top list. The embed title and leaderboard dropdown option no longer use the globe emoji; only **1st–3rd** place show medal emojis (🏆 🥈 🥉), not ranks 4+.
