...
def execute_card(event, action, target_user, name_query)
  ...
  when 'add', 'give'
    ...
    send_cv2(event, [{ type: 17, accent_color: 0x00FF00, components: [
      ...
      { type: 14, spacing: 1 },
      { type: 10, content: "Added **#{real_name}** to #{target_user.mention}'s collection!#{family_remark(event.user.id, 'dev')}" }
    ]}])
  ...
  when 'remove', 'take'
    ...
    send_cv2(event, [{ type: 17, accent_color: 0x00FF00, components: [
      ...
      { type: 14, spacing: 1 },
      { type: 10, content: "Removed one copy of **#{real_name}** from #{target_user.mention}.#{family_remark(event.user.id, 'dev')}" }
    ]}])
  ...
  when 'giveascended', 'addascended'
    ...
    send_cv2(event, [{ type: 17, accent_color: 0x00FF00, components: [
      ...
      { type: 14, spacing: 1 },
      { type: 10, content: "Successfully granted an **Ascended #{real_name}** to #{target_user.mention}!#{family_remark(event.user.id, 'dev')}" }
    ]}])
  ...
  when 'takeascended', 'removeascended'
    ...
    send_cv2(event, [{ type: 17, accent_color: 0x00FF00, components: [
      ...
      { type: 14, spacing: 1 },
      { type: 10, content: "Removed one #{EMOJI_STRINGS['neonsparkle']} star from #{target_user.mention}'s **#{real_name}**.#{family_remark(event.user.id, 'dev')}" }
    ]}])
