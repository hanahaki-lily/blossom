...
def execute_setxp(event, action, target_user, amount)
  ...
  when 'add'
    ...
    send_cv2(event, [{ type: 17, accent_color: 0x00FF00, components: [
      ...
      { type: 14, spacing: 1 },
      { type: 10, content: "Added **#{amount.abs}** XP to #{target_user.mention}.\nThey're now **Level #{new_level}** with **#{new_xp}** XP.#{family_remark(event.user.id, 'admin')}" }
    ]}])
  ...
  when 'set'
    ...
    send_cv2(event, [{ type: 17, accent_color: 0x00FF00, components: [
      { type: 10, content: "## #{EMOJI_STRINGS['developer']} Admin Override" },
      { type: 14, spacing: 1 },
      { type: 10, content: "Set #{target_user.mention}'s total XP to **#{amount.abs}**.\nThat puts them at **Level #{new_level}** with **#{remaining}** XP.#{family_remark(event.user.id, 'admin')}" }
    ]}])
  ...
  when 'level'
    ...
    send_cv2(event, [{ type: 17, accent_color: 0x00FF00, components: [
      { type: 10, content: "## #{EMOJI_STRINGS['developer']} Admin Override" },
      { type: 14, spacing: 1 },
      { type: 10, content: "Set #{target_user.mention}'s level to **#{new_level}**. XP progress preserved.#{family_remark(event.user.id, 'admin')}" }
    ]}])
  end
end
